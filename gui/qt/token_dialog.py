...
class TokenAddLayout(QGridLayout):
    def __init__(self, dialog, callback):
        ...
        if isinstance(self.dialog.parent().wallet.keystore, TrezorKeyStore):
            ...
            #self.dialog.reject()   #xiugai wang1
            #return
...
class TokenAddDialog(QDialog, MessageBoxMixin):
    ...
    def save(self, contract_addr, bind_addr):
        try:
            ...
            symbol = r.get('symbol')
            if not name or not symbol or (decimals is None) or not isinstance(decimals, int): #or not (decimals is null)
                self.show_message('token info not valid: {} {} {}'.format(name, symbol, decimals))
                return
            #self.show_message('name1 ')
            token = Token(contract_addr, bind_addr, name, symbol, decimals, 0)
            #self.show_message('name2 ')
...
class TokenSendDialog(QDialog, MessageBoxMixin):#
    ...
    def do_send(self, pay_to, amount, gas_limit, gas_price):
        with open('./debug_info.txt','a') as f:
                try:
                    f.write('-3 token_amount:do_send' + str(amount) + '\n')
                    f.write('-3 pay_to:do_send' + str(pay_to) + '\n')
                    f.write('-3 self:do_send' + str(self) + '\n')
                except:
                    f.write('-3 token_amount:do_send:can not get' + str(amount) + '\n')
                    f.write('-3 pay_to:do_send:can not get' + str(pay_to) + '\n')  
                    f.write('-3 self:do_send:can not get' + str(self) + '\n')
